...
view(126, 20), axis equal, axis vis3d, grid on

% ------------------------
% TODO: FILL IN THIS PART

hold off, figure(2), hold on, view(126, 20), axis equal, axis vis3d, grid on
tri = delaunay(points2d_cartesian(1, :), points2d_cartesian(2, :));
trisurf(tri, X, Y, Z);

%
hold off, figure(3), hold on, view(126, 20), axis equal, axis vis3d, grid on
if (~isempty(texture))
    draw_textured_triangles(tri, X, Y, Z, ...
        points2d_cartesian(1, :), points2d_cartesian(2, :), texture', 32);
end
plot3( X, Y, Z, '.');
plot3( camx, camy, camz, 'ro' );
